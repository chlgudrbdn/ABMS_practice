...
for world in worldList:
    ...
    for TRY in range(trycount):
        ...
        vmMonitor = pd.DataFrame(columns=['일수',
                                          ...
                                          'n 일차 구매횟수',
                                          'n 일차 매출'])
        writer = pd.ExcelWriter(scriptName+"world"+str(worldList.index(world))+" Log try " + str(TRY) + ".xlsx")
        for date in range(30):  # from 0 to 29
            ...
            vmMonitor.loc[date] = [str(date) + "일차",
                                   ...
                                   vm.revenueLog[-1]]
            # print(vm.get_state())
        revenueY.append(vmMonitor['n 일차 매출'].values)
        priceX.append(vmMonitor['n 일차 가격'].values)
        weatherX.append(vmMonitor['n 일차 날씨'].values)
    ...
    revenueY = np.asarray(revenueY).flatten().tolist()
    priceX = np.asarray(priceX).flatten().tolist()
    weatherX = np.asarray(weatherX).flatten().tolist()

    df = pd.DataFrame({'revenue': revenueY, 'price': priceX})
    # 데이터의 칼럼명을 변경
    # 회귀분석 수행
    X = df['price']
    y = df['revenue']
    X = sm.add_constant(X)
    result = sm.OLS(np.asarray(y, dtype='float'), np.asarray(X, dtype='float')).fit()
    # result = sm.ols(formula='revenue ~ price', data=df.values).fit()
    # 요약결과 출력
    ...
    df = pd.DataFrame({'revenue': revenueY, 'price': priceX, 'weather': weatherX}, dtype='float')
    # 데이터의 칼럼명을 변경
    # 회귀분석 수행
    X = df[['price', 'weather']]
    y = df['revenue']
    ...
    # 모형의 적합값 출력
    # print('Predicted values', result.predict())
    # 출처: http: // bongury.tistory.com / entry / Python - 다중 - 회귀 - 분석[봉구리밥스]
    revenueY = []
    priceX = []
    weatherX = []
